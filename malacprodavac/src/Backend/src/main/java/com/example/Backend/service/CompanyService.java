...
public class CompanyService {
    ...
    public List<Company> getAllCompanies(Consumer<Company> init) {
    ...
    // vraca kompanije koje nisu moje
    @Transactional(readOnly = true)
    public List<Company> getAllCompaniesNot(Long userId, Consumer<Company> init) {
        Company company = companyRepository.findCompanyByUserId(userId).orElse(null);

        List<Company> companies = new ArrayList<>();
        if(company != null)
            companies = companyRepository.findAllByIdNot(company.getId());
        else
    ...
    @Transactional(readOnly = true)
    public Company getCompanyById(Long companyId, Consumer<Company> init) {
        Optional<Company> company = companyRepository.findCompanyById(companyId);

        if (init != null) {
            company.ifPresent(init);
        }
    ...
    public Company createCompany(CompanyDTO param, Long userId) throws Exception {
        ...
        Street street = streetRepository.findStreetByName(param.getStreetName())
                .orElse(null);
        if(street == null) {
            street = new Street();
            street.setName(param.getStreetName());
            if(city == null) {
                City city1 = cityRepository.findCityByName(param.getCityName())
                        .orElse(null);
                street.setCity(city1);
            }
            else {
                street.setCity(city);
            }
        ...
        if(street == null) {
            Street street1 = streetRepository.findStreetByName(param.getStreetName())
                    .orElse(null);
            newCompany.setStreet(street1);
        }
        else {
        ...
        // ako nije uneo sliku, postavlja se ona defaultna
            ...
            fileSystem.saveImage(String.valueOf(addedCompany.getId()), defaultImg, ImageType.LOGO);
        }
    ...
    @Transactional(readOnly = true)
    public List<Company> getCompanyByUserId(Long ownerId, Consumer<Company> init) {
        List<Company> companies = companyRepository.findCompaniesByUserId(ownerId);
    ...
    @Transactional(readOnly = true)
    public List<Company> getCompaniesByUsers(List<User> users, Consumer<Company> init) {
        List<Company> companies = companyRepository.findCompaniesByUserIn(users);
    ...
    public Map<Long, Double> getRatesForCompanies(List<Company> companies) {
        ...
        for(Company company : companies) {
            // uzimam proizvode te kompanije
            List<Product> products = productRepository.findProductByCompanyId(company.getId());
            List<RatingProduct> ratingProducts = ratingProductRepository.findRatingProductByProductIn(products);

            if(ratingProducts.size() != 0) {
                double sum = ratingProducts.stream()
                        .mapToInt(RatingProduct::getRate).sum();
                Double totalRate = sum / ratingProducts.size();

                ratesMap.put(company.getId(), totalRate);
            }
            else {
                ratesMap.put(company.getId(), 0.0);
            }
        }
